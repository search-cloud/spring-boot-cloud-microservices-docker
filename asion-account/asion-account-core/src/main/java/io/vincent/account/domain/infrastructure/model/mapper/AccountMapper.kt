...
interface AccountMapper {
    ...
    fun create(@Param("account") entity: Account): Long
     ...
     */
    @Update("update account set code=#{account.code}, username=#{account.username}, nick_name=#{account.nickName}, password=#{account.password}, used_password=#{account.usedPassword}, mobile=#{account.mobile}, email=#{account.email}, wechat_id=#{account.wechatId}, enabled=#{account.enabled}, parent_id=#{account.parentId}, status=#{account.status}, last_login_at=#{account.lastLoginAt}, operation_error_count=#{account.operationErrorCount}, updated_at=now() where id=#{account.id, jdbcType=BIGINT}")
    fun update(@Param("account") entity: Account): Long
