...
import org.asion.bot.crawler.Constant
import org.asion.bot.crawler.TmallItemDetailPhantomJSCrawlerFactory
...
@Controller
@RequestMapping("/seek/item/detail/phantomjs")
class TmallItemDetailPhantomJSCrawlerController {

    @Autowired
    lateinit var tmallItemCrawlerFactory: TmallItemDetailPhantomJSCrawlerFactory

    @GetMapping("/")
    @ResponseBody
    fun seekItem(): YTXResponse {

        val crawlStorageFolder = "/Users/Asion/Workstation/Work/ytx/Workspace/ytx-parent/ytx-seeker/data/detail"
        val numberOfCrawlers = 5

        val config = CrawlConfig()
        config.crawlStorageFolder = crawlStorageFolder
        config.maxDepthOfCrawling = 0
        config.politenessDelay = 3000
...
//                , BasicHeader("connection", "keep-alive")
        ...
        seekItemList.forEach {
            ...
            RedisClientLocal.set(originId, it, Constant.startDbIndex)
            println(originId)
        }
         ...
         */
        controller.start(tmallItemCrawlerFactory, numberOfCrawlers)

        return YTXResponse.builder().addData("seedUrls", "https://detail.tmall.hk/item.htm?spm=a220m.1000858.0.0.767c2420x1wsEJ&id=520928380305&skuId=3561403886285&areaId=330106&is_b=1&user_id=2541315686&cat_id=55916001&rn=6e62dabd737a68f19303c5871327973e").build()
    }
}